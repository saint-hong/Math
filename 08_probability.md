...
=======
- 확률이 어디에 어느 정도 분포되었는지를 수학적으로 명시하고 명확하게 전달하기 위한 도구
- 종류
   - 확률질량함수, 누적분포함수, 확률밀도함수

### 확률분포
- ``확률분포 probability distribution`` :  어떤 사건에 어느 정도의 확률이 할당되었는지 묘사한 정보
- 확률은 어떤 사건 event (부분집합)에 숫자를 할당하는 함수와 같다. 즉 사건과 사건이 발생할 확률을 관계지어 준다.
- 확률분포는 이러한 사건과 확률의 분포를 보여준다. 

### 확률질량함수
- ``확률질량함수 probability mass function PMF`` : 유한개의 단순사건에 대한 확률을 정의하는 함수
  - p(a) : 소문자 p 사용, 대상은 사건이 아닌 표본이다.
- ``단순사건 elementary event, atomic event`` : 표본이 하나인 사건
  - 단순사건끼리는 서로 교집합을 가지지 않는다. 교집합이 공집합이다.
  - 유한개일 때 콜모고로프의 3번째 공리에 의해서 다른 모든 사건의 확률값을 계산할 수 있다.
    - P({1}) = 0.2, P({2}) = 0.3, P({3}) = 0.5 일때, P({1,2}) = 0.2 + 0.3, P({1, 3}) = 0.2 + 0.5...
  - 모든 단순사건의 합 = 1
- 확률질량함수의 특징 
  - p(a)는 사건 {a}가 아닌 표본 a에 대한 확률이다.
  - 확률 P({a})는 a라는 사건에 대한 확률을 할당한다.
  - P({a}) = p(a), P({1}) = 0.2, p(a) = 0.2
  - P({1, 2})일 경우 p(1, 2)는 틀린식이다. 확률질량함수는 원소가 2개 이상인 사건에 대해서는 확률을 정의할 수 없다.
- 조작 된 주사위에서 p(6)의 확률은 0.5 이고 나머지 숫자들의 확률은 0.1일때의 다른 사건의 확률
  - P({1, 2}) = P({1}∪{2}) = P({1}) + P({2})  = 0.1 + 0.1 = 0.2
  - P({4, 5, 6}) = P({4}∪{5}∪{6}) = P({4}) + P({5}) + P({6}) = 0.1 + 0.1 + 0.5 = 0.7

### 표본수가 무한한 경우의 확률질량함수
- 표본의 수가 무한한 경우는 각 사건에 대해 확률을 직접 할당해야한다. 
- 만약 각 사건이 단순사건이고 확률이 모두 동일하다면, 표본공간의 확률이 1보다 크기 때문이다.
- 확률을 정의할 때 입력변수를 사건으로 정한 이유는 표본의 수가 무한한 경우에 대해서도 확률값을 할당하기 위해서이다.
  - 원반에 화살을 쏜 후 화살이 박힌 각도를 결정하려고 할때 모든 각도에 대해서 확률은 0이다.
  - 왜냐하면 모든 각도가 나올 확률 x로 같다면 전체 표본 집합의 확률이 1보다 크기 때문에, 이런경우는 모든 사건이 0이다.
  - P({θ = 0}) = 0, P({θ = 60}) = 0, ...
  - 따라서 표본의 수가 무한개인 집합의 확률은 모든 사건에 확률값을 직접 할당해야 한다.
  - P({0 < θ <= 30}) = 1/12, P({30 < θ <= 60}) = 1/12, P({0 < θ <= 60 or 90 < θ <= 150}) = (1/12 + 1/12) + (1/12 + 1/12) = 1/3 
...
>>>>>>> d9df2f1488bf282c9f7f8ec773bab17323b719ba
