...
# 결합확률과 조건부확률
- 조건부확률의 의미 : 데이터로부터 어떤 결론을 얻을 수 있는가를 나타내는 숫자

### 베이지안 확률론
- 베이지안 확률론 : 두 사건 A와 B의 관계를 알고 있을 때, 사건 B의 발생으로부터 기존에 알고 있던 사건 A에 대한 확률 P(A)를 좀 더 정확하게 바꿀 수 있는 방법이다. 
    - 베이지안 확률론은 근대 논리학의 관점에서 확률을 규정한다. 
    - 데이터분석의 최종 목적은 데이터가 이야기하는 참과 거짓, 또는 신뢰도를 알아내는 것이다. 베이지안 확률론은 이를 판단할 수 있는 근거가 된다. 
...
- ```조건부확률을 구하기 위해선 결합확률을 알아야 한다.```
    - 결합확률은 사건 A의 확률이나 사건 B의 확률과 무관한 외부지식 또는 정보이다.
...
### 확률변수 
- ```확률변수 probability variable``` : 확률적인 숫자값을 출력하는 변수
    - 알파벳 대문자로 표기 : X, Y
    - X = 0은 사건 A (범인이 남자인 사건)
    - X = 1은 사건 A^c (범인이 여자인 사건)
    - Y = 0은 사건 B (범인이 긴머리인 사건)
    - Y = 1은 사건 B^c (범인이 짧은머리인 사건)
    - P(X) : 확률변수의 확률
...
- 따라서 데이터가 주어지기 전에 이미 어느정도 확률값을 예측하고 있을 때 이를 새로 수집한 데이터와 합쳐서 최종 결과에 반영할 수 있다. 
- 데의터의 개수가 부족할 때 아주 유용하게 사용된다.
- 매일 추가적으로 데이터를 얻을 때 전체 데이터를 분석할 필요없이 어제 분석결과에 새로 들어온 데이터를 합쳐서 업데이트하면 된다. 
...
- ```베이즈 정리의 의미```
    ...
    - P(B|A) : 가능도 likely hood, 사건 A가 발생한 경우 사건 B의 확률 
    - P(B) : 정규화 상수 nomalizing constant, 증거 evidence, 확률의 크기 조정
- 베이즈 정리는 사건 B가 발생함으로써 사건 A의 확률이 어떻게 변화하는지를 알려준다. 따라서 **베이즈 정리는 새로운 정보가 기존의 추론에 미치는 영향을 알 수 있게 해준다.**
